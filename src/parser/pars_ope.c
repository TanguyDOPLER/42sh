...
static struct ast *pipeline(enum parser_status *status, struct lexer *lexer) {
  ...
  ast->left = command(status, lexer);  // on aura forcement un "command"
                                       // au début. Donc
 ...
 */
struct ast *
and_or(enum parser_status *status,
       struct lexer *lexer) { // on créé de base un AST_AND. Si c'est un OR on
                              // change son type. Sinon on touche pas
  if (*status == PARSER_UNEXPECTED_TOKEN) // cas où le parse crash
    return NULL;

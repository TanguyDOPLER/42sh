...
struct ast *command(enum parser_status *status, struct lexer *lexer) {
      ...
      token.use == TOKEN_LEFT_ACC) // cas shell command
  {
    ...
    if (*status != PARSER_OK) {
      ast_free(shell_com);
      fprintf(stderr, "command : error in shell_command\n");
      return NULL;
    }
    struct ast *n = shell_com; // noeud courant dans l'ast
    token = lexer_peek(lexer);
    while (token.use == TOKEN_IONUMBER || (token.type >= TOKEN_REDIR_INPUT &&
      ...
      // premiere redirection trouvée
      if (!(n->type >= AST_REDIR_INPUT && n->type <= AST_REDIR_DOUBLE)) {

...
char *add_slash_at_end(char *str) { // add a '/' at the end if there is not
  if (str == NULL)
    return NULL;
  if (*str == '\0') {
    str = realloc(str, 2);
...
static char *canonic_shape(char *path) {
  ...
  while ((token = strtok_r(rest, "/", &rest)) != NULL) {
    if (strcmp(token, ".") == 0 || token[0] == '\0') {
      ...
      canonical = realloc(canonical, (strlen(canonical) + strlen(token) + 1) *
                                         sizeof(char));
  ...
  if (canonical[0] != '\0' && canonical[strlen(canonical) - 1] != '/') {
    ...
    if (access(canonical, F_OK) == 0) {
      canonical = realloc(canonical, (strlen(canonical) + 2) * sizeof(char));
...
int if_for_rule_4(char *directory_operand) {
  return !(directory_operand[0] == '.' &&
           (directory_operand[1] == '\0' ||
            (directory_operand[1] == '.' &&
             (directory_operand[2] == '/' || directory_operand[2] == '\0')) ||
            directory_operand[1] == '/'));
}
...
int builtin_cd(char *directory_operand) { // faut prendre le SCL pour comprendre
                                          // les étapes ici
  ...
  char *curpath = calloc(strlen_(directory_operand) + strlen_(pwd_env) + 1, 1);
  ...
  setenv("OLDPWD", value_of_variable("OLDPWD"), 1); // change var OLDPWD
